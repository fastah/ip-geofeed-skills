...
def render_html_analysis(analysis: Analysis) -> str:
    ...
    if not analysis.findings and analysis.statistics.invalid_rows == 0:
        success_banner_html = (
            f'<div class="success" role="status"><strong>✅ Nice — your feed is clean.</strong> '
            f"All {analysis.statistics.data_rows:,} rows parsed, every prefix is well-formed, "
            "and we found nothing that would send a visitor to the wrong city. "
            "Ship it with confidence. Happy hacking! :)</div>"
        )
    elif analysis.findings and analysis.statistics.invalid_rows == 0:
        rows_to_sharpen = sum(1 for row in analysis.rows if row.finding_ids)
        success_banner_html = (
            f'<div class="success" role="status"><strong>{rows_to_sharpen:,} rows '
            "could geolocate better.</strong> None of them are broken — they'll parse "
            "fine — but here's what we'd tweak to make them sharper. You decide what "
            "changes.</div>"
        )
    else:
        success_banner_html = ""
...
<div class="hero"><div class="top-inner"><div><div class="eyebrow">// tuning-geofeeds analysis</div><h1>Geofeed quality dashboard</h1><p>Here's what your geofeed says, what our optional checks found, and what's safe to change. We never touch your original file — every fix is yours to approve.</p></div></div></div>
<main id="main" class="container"><div id="summary-stats" class="stats" aria-label="Executive summary"></div>{success_banner_html}<div class="notice" role="note"><strong>How to read this →</strong> location suggestions from external sources are shown side-by-side with your declarations and reach a corrected file only after you approve specific proposals.</div>
<details class="glossary"><summary>Row states and source format</summary><p>{row_state_glossary_html}</p><p>{source_format_html}</p><p>A quick heads-up on reading these: radius and population weight tell you about the <em>place</em>, not how sure we are it's right. And RDAP consistency shows the registration lines up — it's not proof of who legally owns the block.</p></details>
...
<section aria-labelledby="geography-title"><h2 id="geography-title">Geographic evidence</h2><p class="section-intro">Every point on the map comes straight from a real place match. We never invent coordinates or draw boundaries we can't back up.</p><div id="map-status" class="map-status" role="status"></div><div class="map-grid"><div id="map-shell" class="map-shell"><div id="map" aria-label="Interactive geographic evidence map"></div></div><aside class="card"><h3>Textual map equivalent</h3><div id="geo-list" class="geo-list"></div></aside></div></section>
<section aria-labelledby="relationships-title"><h2 id="relationships-title">Relationship evidence</h2><p class="section-intro">Where your prefixes overlap, nest, or duplicate each other — and anywhere two of them disagree about location. These are the relationships worth a second look.</p><div class="card"><div id="relationship-list" class="relationship-list"></div></div></section>
<section aria-labelledby="corrections-title"><h2 id="corrections-title">Correction proposals</h2><p class="section-intro">These are suggestions, nothing more. Your feed doesn't change until you say so — approve the ones you like, skip the rest. We keep a record of exactly what you approved.</p><div class="card"><div id="correction-list" class="relationship-list"></div></div></section>
<section aria-labelledby="rows-title"><h2 id="rows-title">Feed rows</h2><p class="section-intro">Search and filter your rows. Each one shows what you wrote, what we normalized it to, what the place check and RDAP thought, and any findings tied to it — side by side.</p><div class="toolbar"><div class="field"><label for="row-search">Search your rows</label><input id="row-search" type="search" placeholder="Row ID, prefix, country, region, or city"></div><div class="field"><label for="state-filter">Row state</label><select id="state-filter"><option value="">All states</option><option value="invalid">Invalid</option><option value="valid_unresolved">Valid unresolved</option><option value="valid_do_not_geolocate">Do not geolocate</option><option value="not_applicable">Not applicable</option></select></div><div class="field"><label for="severity-filter">Worst finding</label><select id="severity-filter"><option value="">All severities</option><option value="error">Error</option><option value="warning">Warning</option><option value="info">Info</option><option value="none">No findings</option></select></div></div>
...
<section aria-labelledby="downloads-title"><h2 id="downloads-title">Downloads</h2><p class="section-intro">Everything here downloads straight from this page — generated locally, nothing phones home.</p><div class="downloads"><button id="download-json" class="button primary" type="button">Download analysis JSON</button><button id="download-geojson" class="button" type="button">Download GeoJSON</button></div></section>
<section class="legal" aria-labelledby="sources-title"><h2 id="sources-title">Data sources and limits</h2><p>Place data comes from <a href="https://www.geonames.org/" rel="noreferrer">GeoNames</a>. The map is <a href="https://www.openstreetmap.org/copyright" rel="noreferrer">OpenStreetMap</a> via Leaflet — those outlines are hints from the place search, not exact borders. And remember: RDAP tells you the registration is consistent, not who owns the block.</p></section>
<footer><span>Schema {analysis.schema_version} · Source SHA-256 <code>{analysis.source.sha256}</code></span><span>Built only from your validated analysis — no live calls.</span></footer></main>
